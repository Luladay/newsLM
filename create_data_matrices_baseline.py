...
def build_matrices_helper(data_filename, vocab_dict, embed_matrix):
	...
	data_matrix = np.zeros((n_articles, util.glove_dimensions + len(news_names))) # do I need this?? + len(newsnames)
	row_num = 0	
	...
	for i, news_source in enumerate(news_names):
		...
		for j, article in enumerate(all_articles):
			...
			if len(article) > 0:
				...
				if np.isnan(article_avg_vec).any():
					print "Found nan at article ", j
					print "article vectors matrix slice ", embed_matrix[indices]
					print article_avg_vec
					print "article ", article

				labels = np.zeros(len(news_names))
				labels[i] = 1
				# instead of inserting i, insert labels
				# article_avg_vec = np.insert(article_avg_vec, util.glove_dimensions, i) 
